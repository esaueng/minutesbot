...
export function buildSummaryPrompt(input: SummaryInput): string {
  ...
  return [
    ...
    "JSON shape: {\"meetingType\":\"weekly_spqrc\"|\"weekly_sales\"|\"plant_meeting\"|\"general\",\"meetingNotes\":[{\"heading\":\"string\",\"overview\":\"string\",\"items\":[{\"title\":\"string\",\"detail\":\"string\"}]}],\"followUpTasks\":[{\"title\":\"string\",\"description\":\"string\",\"owners\":[\"string\"],\"dueDate\":\"string\"}],\"summary\":string[],\"decisions\":string[],\"actionItems\":[{\"owner\":\"string\",\"task\":\"string\",\"dueDate\":\"string\"}],\"openQuestions\":string[],\"risks\":string[],\"followUps\":string[]}",
    "Transcript:",
...
function buildBaseInstructions(customPrompt?: string): string {
  return [
    ...
    "You are generating a Microsoft Teams-style AI meeting recap for WGS. The recap should read like a structured Teams-generated recap, not a minimal bullet list.",
    "Rendered recap emails and meeting detail views must display this disclaimer at the top: Generated by AI. Be sure to check for accuracy.",
    ...
    "Write meetingNotes as grouped topical notes. Each meetingNotes heading should represent a major area of the meeting. Under each heading, create specific titled items with paragraph details.",
    "Each meetingNotes heading should end with a colon. Each subtopic title should be concise and each detail should be a complete paragraph, usually 1 to 2 sentences.",
    "Include 2 to 7 subtopic items under each major heading when the transcript supports them. Include speaker names, plant names, customer names, programs, systems, locations, deadlines, and metrics when stated.",
